...
static void
DMA_SetConf(DMA_TypeDef *DMAx, uint32_t Stream, uint32_t SrcAddress, uint32_t DstAddress, uint32_t DataLength)
{
...
void UB_VGA_Screen_Init(uint16_t vga_x, uint16_t vga_y)
{
  ...
  VGA.dma2_cr_reg = LL_DMA_GetRegiter_CR(DMAx, Stream); // VGA.dma2_cr_reg = DMA2_Stream5->CR;
}
...
void VGA_horizontal_sync(DMA_TypeDef *DMAx, uint32_t Stream, uint16_t v_line_beg, uint16_t v_line_end)
{
  ...
  {
    VGA.hsync_cnt = 0;
    VGA.start_adr = (uint32_t)VGA_RAM1; // Adresspointer first dot
  }

  if(VGA.hsync_cnt < VGA_VSYNC_IMP) LL_GPIO_ResetOutputPin(VGA_VSYNC_GPIO_Port, VGA_VSYNC_Pin);
  else LL_GPIO_SetOutputPin(VGA_VSYNC_GPIO_Port, VGA_VSYNC_Pin);

  // Test for DMA start
  if((VGA.hsync_cnt > v_line_beg) && (VGA.hsync_cnt < v_line_end))
  {
    // after FP start => DMA Transfer
    LL_DMA_SetRegiter_CR(DMAx, Stream, VGA.dma2_cr_reg);
    LL_DMA_SetMemoryAddress(DMAx, Stream, VGA.start_adr);
    LL_TIM_EnableCounter(TIMx);
    LL_DMA_EnableStream(DMAx, Stream);

    // if(VGA.hsync_cnt == (v_line_end - v_line_beg)/2)
    //   VGA.start_adr = (uint32_t)VGA_RAM1;

    // if(VGA.hsync_cnt & 0x01) // Test Adrespointer for high
    if(!(VGA.hsync_cnt % 2)) // Test Adrespointer for high
...
// void TIM3_IRQHandler(void) { VGA_horizontal_sync(DMA2, 5, VGA_VSYNC_BILD_START, VGA_VSYNC_BILD_STOP); }
