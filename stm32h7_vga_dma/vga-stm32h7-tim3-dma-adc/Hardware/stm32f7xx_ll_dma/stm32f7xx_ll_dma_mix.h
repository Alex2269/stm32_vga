...
#include "stm32f7xx_ll_dma_flags.h"
...
void LL_DMA_SetRegiter_CR(DMA_TypeDef *DMAx, uint32_t Stream, uint32_t Configuration)
{
  ...
  WRITE_REG(((DMA_Stream_TypeDef *)((uint32_t)((uint32_t)DMAx + LL_DMA_STR_OFFSET_TAB[Stream])))->CR, Configuration);
}
...
uint32_t LL_DMA_GetRegiter_CR(DMA_TypeDef* DMAx, uint32_t Stream)
{
  ...
  return (READ_REG(((DMA_Stream_TypeDef*)((uint32_t)((uint32_t)DMAx + LL_DMA_STR_OFFSET_TAB[Stream])))->CR));
}
